				...
				<div class="container">
					...
					<div id="page-area">
						<c:if test="${pv.currentPage > 1}">
							<a class="btn btn-primary btn-lg" href="${root}/faq/list?page=${pv.currentPage-1}">이전</a>
						</c:if>
						<c:forEach begin="${pv.startPage}" end="${pv.endPage}" step="1"	var="i">
							<c:if test="${pv.currentPage != i}">
								<a class="btn btn-primary btn-lg" href="${root}/faq/list?page=${i}">${i}</a>
							</c:if>
							<c:if test="${pv.currentPage == i}">
						...
						<c:if test="${pv.currentPage < pv.maxPage}">
							<a class="btn btn-primary btn-lg" href="${root}/faq/list?page=${pv.currentPage+1}">다음</a>
						</c:if>
					</div>
				</div>
			</div>
